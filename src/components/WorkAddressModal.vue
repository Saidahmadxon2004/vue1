      ...
      <div class="modal-dialog modal-lg">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Ish joyi Manzili</h5>
            ...
            <table class="table table-striped">
              ...
              <tbody>
                <tr v-for="item in filteredItems" :key="item.id">
                  <td @click="selectItem(item)" style="cursor: pointer">
                    {{ item.name }}
                  </td>
                  <td>
  ...
  methods: {
    ...
    saveItem() {
      ...
      const url = this.editMode 
        ? `/api/v1/crm-work-address/id/${this.currentItem.id}`
        : '/api/v1/crm-work-address';
    ...
    deleteItem(id) {
      if (confirm("Haqiqatan ham o'chirmoqchimisiz?")) {
        this.$axios.delete(`/api/v1/crm-work-address/id/${id}`)
          .then(() => this.$emit('refresh'));
      }
    },
    selectItem(item) {
      this.$emit('select', item.id);
      this.$emit('close'); // tanlash bn modal yopiladi
    }
  }
}
</script>
