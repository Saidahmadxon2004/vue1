<template>
  <form class="card p-4 shadow-sm border rounded-3">
    <h5 class="mb-4">Country Qo‘shish / Tahrirlash</h5>
    ...
    <div class="row mb-3">
      <div class="col-md-6">
        ...
        <input
          ...
          class="form-control"
          placeholder="Country nomini yozing ..."
          required
        />
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      Send() {
        let self = this;
        let method='post';
        var url='api/v1/country';
        if(self.$route.params.id){
        method='patch';
        url='api/v1/country/id/'+self.$route.params.id;
        }
        self.$axios({
          method: method,
          url: url,
          ...
          self.$router.push({ path: "/countrys" });
        }).catch(error => {
          if (error.response) {
