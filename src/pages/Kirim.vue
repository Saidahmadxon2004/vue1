        ...
        <tbody>
          <tr v-for="(item, index) in data" :key="index">
            <td>{{ index + 1 }}</td>
             ...
             <td>{{ item.summa }}</td>
               <td>{{ item.prixod_summa}}</td>
                <td>{{ item.pay_type?.name}}</td>
            <!-- \
              -->
            <td>
  <div class="action-buttons-container">
    <button class="action-btn btn-edit" @click="$router.push('/kirim/update/' + item.id)">
...
export default {
  ...
  async mounted() {
    try {
      const res = await this.$axios.get('/api/v1/prixod');
      this.data = res.data
  ...
  methods: {Dates(date) {
    ...
    async deleteElement(id) {
      try {
        await this.$axios.delete(`/api/v1/prixod/id/${id}`);
